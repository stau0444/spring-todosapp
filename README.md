...
#### JUnit , 자바를 위한 테스트 프레임워크

- 개발자는 JUnit이 정의한 규칙에 따라 테스트할 코드를 메서드로 작성한다.
- 테스트가 실행되면 JUnit이 테스트 객체를 생성하고 , 개발자가 작성한 테스트 코드를 실행한다.
- 통합개발환경부터 빌드 도구까지 다양한 개발 도구와 통합해서 사용된다.
- 사실상 자바의 표준 테스트 프레임워크라고 불릴 만큼 폭넓게 사용되고 있다.
- JUnit의 TestRunner는  @Test가 붙어있는 메서드를 테스트 실행 대상으로 인지한다.

#### 테스트와 스프링프레임 워크

- 스프링의 POJO와 의존관계 주입을 이용한 프로그래밍 모델이 제공하는 큰 혜택 중 하나가 테스트이다.
- POJO 프로그래밍 모델은 단위 테스트를 손쉽게 작성할 수 있는 환경을 제공한다.
- 스프링 컨테이너만으로 DB를 포함 다양한 외부 리소스를 이용한 이상적인 통합 테스트를 할 수 있다.
- 단위 테스트보다 실행 속도가 느리고 ,무겁다
- 실 운영환경과 유사한 구성으로 동작하는 통합 테스트를 손쉽게 만들 수 있다.
- 테스트간 스프링 컨테이너 캐싱을 관리한다.
- 테스트 대상이 필요로 하는 의존관계 주입을 제공한다.
- 통합 테스트에 적합한 트랜잭션 관리를 제공한다.
- 통합 테스트 작성을 돕는 다양한 스프링 - 전용 클래스를 제공한다


#### MockMVC

- 스프링 MVC 애플리케이션의 요청부터 응답까지 모든 과정을 테스트 할 수 있다.
- 서블릿 컨테이너를 사용하지 않고 모의 요청 및 응답 객체를 통해 테스트를 수행할 수 있다.
- 단일 컨트롤러 또는 애플리케이션 컨텍스트를 대상으로 MockMVC 객체를 구성 할 수 있다.

#### WebTestClient

- 스프링 MVC 부터 WebFlux까지 스프링 기반 서버 애플리케이션을 테스트 할 수 있따.
- 실 서버 대상으로 E2E 테스트와 모의 요청 및 응답 객체를 이용한 테스트 모두 지원한다.
- 서버나 단일 컨트롤러 또는 애플리케이션 컨텍스트를 대상으로 WebTestClient 객체를 구성 할 수 있다.